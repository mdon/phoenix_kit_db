defmodule PhoenixKitDb.ActivityLogAssertions do
  @moduledoc """
  Helpers for asserting activity log entries landed with the right
  action, module, and metadata shape.

  The DB module itself doesn't log activity entries (read-mostly +
  system-level), so these helpers exist for parity with other modules
  and for any tests that need to verify activity rows from neighbouring
  features.
  """
  ...
  given match criteria. Returns the matched row.
